...
TEST_CASE( "non_null_function - take() and re-wrap", "[non_null_function]" )
{
    nova::non_null_function< int() > fn1( [] {
        return 7;
    } );
    auto                             fn2 = nova::non_null_function< int() >( take( std::move( fn1 ) ) );
    CHECK( fn2() == 7 );
}
...
TEST_CASE( "non_null_move_only_function - underlying() accessor", "[non_null_move_only_function]" )
{
    ...
    CHECK( const_cast< std::move_only_function< int( int ) >& >( underlying )( 77 ) == 77 );
}
